...
use recrypt::api_480::{
    CryptoOps, DefaultRng, Ed25519, Ed25519Ops, Ed25519Signature, Hashable, KeyGenOps, Plaintext,
    PrivateKey, PublicSigningKey, RandomBytes, Recrypt480, SchnorrOps, SchnorrSignature, Sha256,
    Sha256Hashing, SigningKeypair,
};
use std::sync::Mutex;
...
impl Api480 {
    ...
    pub fn generateTransformKey(
        ...
        let transform_key = self
            ...
            .generate_transform_key(
                &PrivateKey::new(util::slice_to_fixed_60_bytes(
                    from_private_key,
                    "privateKey",
                )),
    ...
    pub fn computePublicKey(&mut self, private_key: &[u8]) -> Result<JsValue, JsError> {
        let computed_public_key = self
            .api
            .compute_public_key(&PrivateKey::new(util::slice_to_fixed_60_bytes(
                private_key,
                "privateKey",
            )))
            .map_err(WasmError::new)?;
    ...
    pub fn deriveSymmetricKey(&mut self, plaintext: &[u8]) -> Vec<u8> {
        let symmetric_key =
            self.api
                .derive_symmetric_key(&Plaintext::new(util::slice_to_fixed_720_bytes(
                    plaintext,
                    "plaintext",
                )));
    ...
    pub fn encrypt(
        ...
        let encrypted_value = self
            .api
            .encrypt(
                &Plaintext::new(util::slice_to_fixed_720_bytes(plaintext, "plaintext")),
                &util::js_object_to_public_key(&to_public_key_obj)?,
                &SigningKeypair::from_bytes(&util::slice_to_fixed_64_bytes(
                    private_signing_key,
                    "privateSigningKey",
    ...
    pub fn decrypt(
        ...
        let decrypted_value = self
            ...
            .decrypt(
                util::js_object_to_encrypted_value(encrypted_value_js)?,
                &PrivateKey::new(util::slice_to_fixed_60_bytes(private_key, "privateKey")),
            )
            .map_err(WasmError::new)?;
    ...
    pub fn schnorrVerify(
        ...
        Ok(self.api.schnorr_verify(
            ...
            augmented_private_key
                .map(|v| PrivateKey::new(util::vector_to_fixed_60_bytes(&v, "augmentedPrivateKey")))
                .as_ref(),
            &message.to_vec(),
            SchnorrSignature::new(util::slice_to_fixed_120_bytes(signature, "signature")),
        ))
    }
}
...
pub fn augmentTransformKey480(
    ...
    let augmented_transform_key = util::js_object_to_transform_key(transform_key_js)?
        .augment(&PrivateKey::new(util::slice_to_fixed_60_bytes(
            private_key,
            "privateKey",
        )))
        .map_err(WasmError::new)?;
...
pub fn addPrivateKeys(private_key_a: &[u8], private_key_b: &[u8]) -> Result<Vec<u8>, JsError> {
    let pubKeyA = PrivateKey::new(util::slice_to_fixed_60_bytes(private_key_a, "privateKeyA"));
    let pubKeyB = PrivateKey::new(util::slice_to_fixed_60_bytes(private_key_b, "privateKeyB"));
